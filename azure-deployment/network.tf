...
resource "azurerm_public_ip" "linux_ucp_manager" {
  ...
  name                         = "${format("%s-Manager-PublicIP-%s", local.dci_id, "${count.index + 1}")}"
  location                     = "${azurerm_resource_group.rg.location}"
  resource_group_name          = "${azurerm_resource_group.rg.name}"
...
resource "azurerm_network_interface" "linux_ucp_manager" {
  count                     = "${var.linux_ucp_manager_count}"
  name                      = "${format("%s-Manager-LinuxNet-%s", local.dci_id, "${count.index + 1}")}"
  location                  = "${azurerm_resource_group.rg.location}"
  resource_group_name       = "${azurerm_resource_group.rg.name}"
  network_security_group_id = "${azurerm_network_security_group.ee_sg.id}"

  ip_configuration {
    name                                    = "${format("%s-Manager-LinuxIP-%s", local.dci_id, "${count.index + 1}")}"
...
resource "azurerm_public_ip" "linux_ucp_worker" {
  ...
  name                         = "${format("%s-Worker-PublicIP-%s", local.dci_id, "${count.index + 1}")}"
  location                     = "${azurerm_resource_group.rg.location}"
  resource_group_name          = "${azurerm_resource_group.rg.name}"
  public_ip_address_allocation = "static"
}
...
resource "azurerm_network_interface" "linux_ucp_worker" {
  count                     = "${var.linux_ucp_worker_count}"
  name                      = "${format("%s-Worker-LinuxNet-%s", local.dci_id, "${count.index + 1}")}"
  location                  = "${azurerm_resource_group.rg.location}"
  resource_group_name       = "${azurerm_resource_group.rg.name}"
  network_security_group_id = "${azurerm_network_security_group.linux_sg.id}"

  ip_configuration {
    name                                    = "${format("%s-Worker-LinuxIP-%s", local.dci_id, "${count.index + 1}")}"
    subnet_id                               = "${azurerm_subnet.subnet.id}"
...
resource "azurerm_public_ip" "windows_ucp_worker" {
  ...
  name                         = "${format("%s-Windows-PublicIP-%s", local.dci_id, "${count.index + 1}")}"
  location                     = "${azurerm_resource_group.rg.location}"
  resource_group_name          = "${azurerm_resource_group.rg.name}"
  public_ip_address_allocation = "static"
}
...
resource "azurerm_network_interface" "windows_ucp_worker" {
  count                     = "${var.windows_ucp_worker_count}"
  name                      = "${format("%s-Worker-WindowsNet-%s", local.dci_id, "${count.index + 1}")}"
  location                  = "${azurerm_resource_group.rg.location}"
  resource_group_name       = "${azurerm_resource_group.rg.name}"
  network_security_group_id = "${azurerm_network_security_group.windows_sg.id}"

  ip_configuration {
    name                                    = "${format("%s-Worker-WindowsIP-%s", local.dci_id, "${count.index + 1}")}"
    subnet_id                               = "${azurerm_subnet.subnet.id}"
...
resource "azurerm_public_ip" "linux_dtr" {
  ...
  name                         = "${format("%s-DTR-PublicIP-%s", local.dci_id, "${count.index + 1}")}"
  location                     = "${azurerm_resource_group.rg.location}"
  resource_group_name          = "${azurerm_resource_group.rg.name}"
  public_ip_address_allocation = "static"
}
...
resource "azurerm_network_interface" "linux_dtr" {
  count                     = "${var.linux_dtr_count}"
  name                      = "${format("%s-DTR-LinuxNet-%s", local.dci_id, "${count.index + 1}")}"
  location                  = "${azurerm_resource_group.rg.location}"
  resource_group_name       = "${azurerm_resource_group.rg.name}"
  network_security_group_id = "${azurerm_network_security_group.ee_sg.id}"

  ip_configuration {
    name                                    = "${format("%s-DTR-LinuxIP-%s", local.dci_id, "${count.index + 1}")}"
    subnet_id                               = "${azurerm_subnet.subnet.id}"
